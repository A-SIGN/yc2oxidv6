 ...
 */
/**
 * Web Service Security format defined
 * 
 * @category Asign
 * @package  Asign_Yellowcube_V_EE
 * @author   Asign <entwicklung@a-sign.ch>
 ...
 * @link     http://www.a-sign.ch/
 */
namespace AsignYellowcube\Core\Utils\Inc;
    ...
    public function setCanonicalMethod($method) {
        switch ($method) {
            ...
            case 'http://www.w3.org/2001/10/xml-exc-c14n#WithComments':
                ...
                break;
            default:
                throw new StandardException('Invalid Canonical Method');
        }
...
/* Support PHP versions < 5.2 not containing C14N methods in DOM extension */
        ...
        if (($php_version[0] < 5) || ($php_version[0] == 5 && $php_version[1] < 2) ) {
            if (! is_null($arXPath)) {
                throw new StandardException("PHP 5.2.0 or higher is required to perform XPath Transformations");
            }
            $c14NGeneral = new C14NGeneral();
            return $c14NGeneral->C14NGeneral($node, $exclusive, $withComments);
        }
    ...
    public function calculateDigest ($digestAlgorithm, $data) {
        switch ($digestAlgorithm) {
            ...
            case XMLSecurityDSig::RIPEMD160:
                ...
                break;
            default:
                throw new StandardException("Cannot validate digest: Unsupported Algorith <$digestAlgorithm>");
        }
        if (function_exists('hash')) {
            ...
            throw new StandardException('xmlseclibs is unable to calculate a digest. Maybe you need the mhash library?');
        }
    }
    ...
    public function getRefIDs() {
        ...
        $nodeset = $xpath->query($query, $this->sigNode);
        if ($nodeset->length == 0) {
            throw new StandardException("Reference nodes not found");
        }
    ...
    public function validateReference() {
        ...
        foreach ($nodeset AS $refNode) {
            if (! $this->processRefNode($refNode)) {
                ...
                throw new StandardException("Reference validation failed");
            }
        }
        return TRUE;
    }
   ...
   public function addObject($data, $mimetype=NULL, $encoding=NULL) {
    ...
    public function verify($objKey) {
        ...
        if (empty($sigValue)) {
            throw new StandardException("Unable to locate SignatureValue");
        }
    ...
    static function staticAdd509Cert($parentRef, $cert, $isPEMFormat=TRUE, $isURL=False, $xpath=NULL) {
          ...
          if (! $parentRef instanceof \DOMElement) {
            throw new StandardException('Invalid parent Node parameter');
          }
