...
class Asign_YellowCubeCore extends FrontendController
{
    /**
    ...
    public function createYCCustomerOrder($oObject)
    {
        try{
            ...
            if(count($oObject->Order->OrderPositions) > 0){
                $oSoap = oxNew(Asign_SoapClientApi::class);
            ...
            Registry::getUtilsView()->addErrorToDisplay(new StandardException($soapex->getMessage()));
        }
    }
    ...
    public function insertArticleMasterData($oRequestData)
    {
        try{
            ...
            Registry::getUtilsView()->addErrorToDisplay(new StandardException(utf8_decode($soapex->getMessage())));
        }
    }
    ...
    public function getYCGeneralDataStatus($soxId, $sType)
    {
        ...
        try{
             ...
             * @START
             * (Description:)
             * Set the exception error while fetching response into database,
             * So that it can be shown correctly in shop admin backend
             * (Date:)
             * 26-September-2019 (06:05 PM PST)
             *
             * */
            $oModel = oxNew(Asign_YellowCube_Model::class);

            $exceptionResponse = [
                'EventTimestamp' => $soapex->getPrevious()->detail->GEN_Response->EventTimestamp,
                'MessageType' => $soapex->getPrevious()->detail->GEN_Response->MessageType,
                'StatusCode' => $soapex->getPrevious()->detail->GEN_Response->StatusCode,
                'StatusType' => $soapex->getPrevious()->detail->GEN_Response->StatusType,
                'StatusText' => $soapex->getPrevious()->detail->GEN_Response->StatusText,
                'Reference' => $soapex->getPrevious()->detail->GEN_Response->Reference,
                'Status' => 'Error',
            ];

            if($soapex->getPrevious()->detail->GEN_Response->MessageType == 'ART') {
                $oTable = "oxarticles";
                $oModel->saveResponseData($exceptionResponse, $oTable, $soxId);
            } else if ($soapex->getPrevious()->detail->GEN_Response->MessageType == 'WAB') {
                $oTable = "oxorder";
                $oModel->saveResponseData($exceptionResponse, $oTable, $soxId, $sType);
            }
            /*
             * @END
             * */
    ...
    {
        $sIsoAlpha = DatabaseProvider::getDb()->getOne("select `oxisoalpha2` from `oxcountry` where `oxid` = '" . $sCountry . "'");
        return $sIsoAlpha;
    }
     ...
     * @return oxUser
     */
    protected function _getShippingAddressUser(Order $oOrder){
     ...
     * @return object
     */
    public function getYCFormattedOrderData(Order $oOrder, $aData = null, $isReturn = false)
    {
        $oObject = new \stdClass();

        // define params needed
        $sLanguage      = Registry::getLang()->getLanguageAbbr($oOrder->oxorder__oxlang->value);
...
//            var_dump($article);
    ...
    public function getYCFormattedArticleData($oArticle, $sFlag, $sBaseOUM = null)
    {
        ...
        foreach ($aLangs as $lang) {
            ...
            $proTitle = DatabaseProvider::getDb()->getOne("select `".$sTitle."` from `oxarticles` where `oxid` = '".$proid."'");
        ...
        try {
            ...
            if (in_array($countryISO, $myArray)) {
                ...
                if (strlen($zipValue) != $maxChars) {
                    error_log("[ ".date("Y-m-d H:i:s")." ] ". Registry::getLang()->translateString('ASIGN_MESSAGE_ZIPCODE_MISMATCH') ." \n", 3, $this->sLogFilepath);
                    Registry::getUtilsView()->addErrorToDisplay(new StandardException(Registry::getLang()->translateString('ASIGN_MESSAGE_ZIPCODE_MISMATCH')));
                } elseif (preg_match('/[A-Za-z]/', $zipValue)) {
                    error_log("[ ".date("Y-m-d H:i:s")." ] ". Registry::getLang()->translateString('ASIGN_MESSAGE_ZIPCODE_INVALID') ." \n", 3, $this->sLogFilepath);
                    Registry::getUtilsView()->addErrorToDisplay(new StandardException(Registry::getLang()->translateString('ASIGN_MESSAGE_ZIPCODE_INVALID')));
                } else {
                    return $zipValue;
                }
            } else {
            ...
            Registry::getUtilsView()->addErrorToDisplay(new StandardException($sEx->getMessage()));
        }
    }
