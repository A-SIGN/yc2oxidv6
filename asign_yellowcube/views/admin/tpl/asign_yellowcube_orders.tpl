...
[{assign var="stylepath" value=$oViewConf->getModuleUrl("asign_yellowcube", "out/css/ycubestyles.css")}]
            ...
            [{if $isManual}]
                <td width="50%">
                    <fieldset>
                        ...
                        [{*
                            WAB Response Status Code < 1  || WAB Response Status Code > 100
                            (Means the order has some error and show the "Send Order To Yellowcube" button
                            again.)
                        *}]
                        [{if $ORDResponse ne ""}]
                            [{if $ORDResponse.StatusCode ne 10 && $WABResponse eq ""}]
                            ...
                            [{else}]
                                [{if $WABResponse ne ""}]
                                    [{if $WABResponse.StatusCode lt 1 || $WABResponse.StatusCode gt 100}]
                                        <button type="submit" name="create" id="icreate" onclick="return validateFileExt(document.myedit.orderpdf.value)">[{oxmultilang ident="ASIGN_YCARTICLES_BUTTON_CREATE"}]</button>
                                    [{/if}]
                                [{/if}]
                            [{/if}]
