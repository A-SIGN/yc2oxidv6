...
namespace AsignYellowcube\Model;
...
class Asign_YellowCube_Model extends MultiLanguageModel
{
    /**
     * Current class name
     *
    ...
    public function getYellowcubeReport($sOxid, $sTable, $isReturn = false, $sColumn = self::YCRESPONSE)
    {
        ...
        $aComplete = DatabaseProvider::getDb(DatabaseProvider::FETCH_MODE_ASSOC)->getOne($sQuery);
    ...
    {
        $aComplete = DatabaseProvider::getDb(DatabaseProvider::FETCH_MODE_ASSOC)->getOne("select `" . self::YCWABRESPONSE . "` from `oxorder` where `oxid` = '" . $sOxid ."'");
        $aResponse = unserialize($aComplete);
    ...
    {
        $aComplete = DatabaseProvider::getDb(DatabaseProvider::FETCH_MODE_ASSOC)->getOne("select `" . self::YCWARRESPONSE . "` from `oxorder` where `oxid` = '" . $sOxid ."'");
        $aResponse = unserialize($aComplete);
    ...
    public function getYCReferenceNumber($sOxid, $sType)
    {
        ...
        if ($sType == "WAR") {
            $sReference = DatabaseProvider::getDb()->getOne("select `oxordernr` from `oxorder` where `oxid` = '" . $sOxid . "'");
        }
    ...
    public function resetInventoryData(){
        $oArticleList = oxNew(ListModel::class);
        $oArticleList->init(Article::class);
        ...
        foreach($oArticleList as $oArticle){
            ...
            if($aResponse['StatusCode'] == 100){
                $oArticle->oxarticles__oxstock = new Field(0);
                $oArticle->save();
            }
        }
    }
    ...
    public function storeFinalData($aData)
    {
        try{
            ...
            Registry::getUtilsView()->addErrorToDisplay(new StandardException('Database Error: ' . $dbEx->getMessage()));
        }
    }
    ...
    {
        $sResponse = DatabaseProvider::getDb()->getOne("select `oxadditional` from `asign_ycinventory` where `oxid` = '" . $soxId . "'");
    ...
    {
        return DatabaseProvider::getDb()->getOne("select `oxname` from `oxshops` where `oxid` = '" . $shopid . "'");
    }
    ...
    {
        $sParam = DatabaseProvider::getDb(DatabaseProvider::FETCH_MODE_ASSOC)->getOne("select `" . self::YCDETAILS . "` from `oxarticles` where `oxid` = '".$sOxid."'");
